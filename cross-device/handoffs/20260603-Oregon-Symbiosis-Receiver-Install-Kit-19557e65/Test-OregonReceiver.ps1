<#
.SYNOPSIS
  Test-OregonReceiver.ps1 — THE VERIFICATION STAR for 19557e65 hardened Oregon receiver packaging.
  ...
  Exits 0 ONLY on full PASS; prints detailed failure + "PASS — matches LIVE_TEST_19557e65_RECEIPTS.md" on success.
...
Write-Host "=== Test-OregonReceiver (19557e65 hardened parity — must match LIVE_TEST_19557e65_RECEIPTS.md) ===" -ForegroundColor Cyan
...
Write-Host "Dropped sample task: $taskFile" -ForegroundColor Green
...
if (-not (Test-Path $pending)) {
    ...
    if ($pcontent -notmatch "Pending prompt — correlation $corr") { Fail "pending artifact missing header" }
...
if (Test-Path $logFile) {
    ...
    Write-Host "(Logs jsonl not at expected $logFile — may be in USERPROFILE or different; non-fatal if corr events visible in stdout above)" -ForegroundColor Yellow
}
...
$bustTask | Out-File -Encoding utf8 $bustFile
Write-Host "Dropped bust task for second --Once..."
& powershell -NoProfile -ExecutionPolicy Bypass -File $Launcher -Once
...
if ($global:TestFailed) {
    Write-Host ""
    Write-Host "TEST FAILED — see FAIL lines above. Does not match LIVE_TEST_19557e65_RECEIPTS.md" -ForegroundColor Red
    exit 1
} else {
    Write-Host ""
    Write-Host "PASS — matches LIVE_TEST_19557e65_RECEIPTS.md exactly (health pre-claim + age, atomic claim+archive to failed/processed, enriched status+version+machine+last_rcs+health_ok, beacon active/bust flags, corr logs/events, pending full fmt on rc-fail, no silent loss)." -ForegroundColor Green
