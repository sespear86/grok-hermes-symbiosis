...
**Repo Kumquat (2026-06-01, explicit "repo kumquat" trigger):** Dedicated pass on the canonical git repo completed. One lingering ~syncthing temp purged from Mempalace/symbiosis/device-presence/. Git status audited (12M + 10?? — all expected recent artifacts + drift; no surprise cruft). 20260601 hygiene receipt enriched with full repo section + exact personal-shell commit commands. .gitignore hygiene confirmed solid. Living top directives (this section) unchanged — the dedicated ingest companion token + rich BustANut parity remain the actionable Oregon items. Repo layer now properly versioned for the one extended machine. Signature per prime. (See the enriched receipt for commands + full filthy details.)
